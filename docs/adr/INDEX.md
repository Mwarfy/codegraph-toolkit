...
| [005](005-detector-pattern-bundle-per-file.md) | Tout détecteur codegraph qui scanne des fichiers TS expose 4 éléments : 1. Un helper pure `extractXxxFileBu... | `packages/codegraph/src/cli/_shared.ts`, `packages/codegraph/src/cli/commands/affected.ts` +21 |
...
| [007](007-salsa-incremental-contracts.md) | `incremental/queries.ts` et `incremental/database.ts` sont les **points d'entrée canoniques** pour toute co... | `packages/codegraph/src/extractors/tainted-vars.ts`, `packages/codegraph/src/incremental/arguments.ts` +34 |
