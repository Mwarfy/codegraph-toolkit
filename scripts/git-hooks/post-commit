#!/bin/bash
# ─── post-commit hook (générique @liby/adr-toolkit) ───
#
# Régénère le snapshot codegraph + le boot brief après chaque commit.
# Installé par `npx @liby/adr-toolkit init` dans `<projet>/scripts/git-hooks/post-commit`.
#
...
if [ -z "$CODEGRAPH_SKIP" ] && [ -f "$REPO_ROOT/codegraph.config.json" ] || [ -f "$REPO_ROOT/codegraph/codegraph.config.json" ]; then
  ...
  npx @liby/codegraph analyze > /dev/null 2>&1 && \
