 ...
 *
 * Exclusion auto-generated followers : CLAUDE-CONTEXT.md, CHANGELOG-RECENT,
 * synopsis* sont régénérés post-commit indépendamment de la source qui
 * change. Du coup P(auto-gen|source.ts) = 100% pour TOUTE source —
 * faux positifs systémiques inutiles. On les skip comme follower mais
 * on les garde comme driver (improbable en pratique mais pas exclu).
 */
const AUTO_GENERATED_FOLLOWERS = new Set([
  'CLAUDE-CONTEXT.md',
  'CHANGELOG-RECENT.md',
  'synopsis.json',
  'synopsis-level1.md',
...
function isAutoGenerated(file: string): boolean {
  if (AUTO_GENERATED_FOLLOWERS.has(file)) return true
  if (file.startsWith('.codegraph/')) return true
  return false
}
...
function pushIfAbove(
  ...
  if (total <= 0) return
  // Skip auto-generated followers : leur P(follower|driver) = 100% pour
  // TOUS les drivers du repo (régénérés post-commit indépendamment de la
  // source qui change). Faux positifs systémiques de COMPOSITE-BAYESIAN-
  // DRIVER : 11/11 NEW violations actuelles sur Sentinel viennent de là.
  if (isAutoGenerated(follower)) return
