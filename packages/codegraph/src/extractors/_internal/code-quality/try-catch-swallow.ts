...
export function extractTryCatchSwallows(
  ...
  for (const tryStmt of sf.getDescendantsOfKind(SyntaxKind.TryStatement)) {
    ...
    if (stmts.length === 0) {
      // Body vide — vérifier la présence d'un commentaire substantiel
      // (≥ 3 chars). `catch { /* best-effort */ }` ou
      // `catch { // intentional }` documentent l'intention → skip.
      ...
      kind = allLog ? 'log-only' : 'no-rethrow'
    }
